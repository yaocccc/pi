...
When user judgment is needed, the main agent calls `ask_question`, then explicitly relays the answer to the Worker; user choices are never forwarded automatically. While the confirmation UI is actually open, only waiting tasks have their remaining task budgets and question deadlines paused. Newly arriving questions join the pause, while independent work continues. Human waiting has a **30-minute cap**; when interaction ends normally, remaining budgets resume rather than reset to their full duration. Reaching the cap is treated as timeout or cancellation, never approval. Queued questionnaires do not start an additional pause, and queue time counts toward their interaction cap.
...
Each batch uses only its original Worker card, styled consistently with the tools in `extensions/ui/`. Follow-up answer, wait, and cancel calls remain in the model transcript but add no visible cards. Task status, tool activity, tier, turns, usage, elapsed time, completion summaries, and per-task Q&A update in place; expand the card for full Q&A. Batch-ID summary lines and routine execution-slot waiting notices are omitted; errors and timeout diagnostics remain visible.
