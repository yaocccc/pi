...
export const TOOL_DESCRIPTION = `Use this tool autonomously for bounded, independently verifiable coding subtasks. Do not ask the user before delegating low-risk tasks. Use Fast for clear local work, Normal for normal development, and Deep for complex or high-risk work. Use Max only when the user explicitly requests Max, xhigh, or maximum strength, and set userExplicitMax: true. Read the worker-orchestration skill when decomposition, parallelization, routing, review, or acceptance strategy is non-trivial. Workers may not create other workers. Failed Worker tasks are returned to the main agent for direct handling and are not automatically retried. The main agent remains responsible for reviewing the diff, validation, and acceptance evidence.`;
